...
/// <summary>
/// Автоматически создаёт и настраивает все объекты для сцены FarmScene при запуске.
/// Просто добавь этот скрипт на любой объект в сцене - он всё настроит сам.
/// </summary>
public class FarmSceneInitializer : MonoBehaviour
{
    ...
    [ContextMenu("Setup Farm Scene")]
    public void SetupFarmScene()
    {
        Debug.Log("Начинаю автоматическую настройку FarmScene...");

        // 1. Создаём или находим GameManager
        IdleFarmManager gameManager = FindObjectOfType<IdleFarmManager>();
        if (gameManager == null)
        {
            GameObject gmObj = new GameObject("GameManager");
            gameManager = gmObj.AddComponent<IdleFarmManager>();
            Debug.Log("Создан GameManager с IdleFarmManager");
        }

        // 2. Создаём или находим SceneManager
        ...
        CreateUIText(canvas.transform, "TapUpgradeCostText", "Upgrade: 10", 20, new Vector2(200, -200));

        // 5. Создаём кнопки
        CreateButton(canvas.transform, "ClickButton", "TAP", new Vector2(0, -300), new Vector2(300, 100));
        CreateButton(canvas.transform, "UpgradeButton", "Upgrade Tap", new Vector2(200, -300), new Vector2(200, 100));
        CreateButton(canvas.transform, "TerraformButton", "Terraform", new Vector2(400, 400), new Vector2(150, 50));

        Debug.Log("Автоматическая настройка FarmScene завершена!");
    }
